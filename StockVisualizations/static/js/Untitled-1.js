...
d3.json("https://dataanalyticsproject.github.io/StockMarketAnalysis/Create%20APIs%20for%20Data/data.json")
  .then(function(data) {
    ...
    //filter on DJI
    var ticker = data.filter(stock => stock.Stock == "ID");
...
function init() {
   ...
   d3.json("https://dataanalyticsproject.github.io/StockMarketAnalysis/Create%20APIs%20for%20Data/data.json").then((importedData) => {
  
      var stock1 = importedData[0];
      console.log(importedData);
      console.log(stock1);
      stock1.forEach((stock)=> {
        dropdownMenu.append("option").text(stock).property("value", stock)
      })
      drawCharts(stock[0])
    });
  }
